...
{
    public class KruskalsOldItsRubbish<T> //Everything MUST be undirected, so make the MST tree undirected, and make the class not run if the graph is undirected.
    {
        //Add another function so that you can return MST length and the MST graph
        AdjacencyList<T> graph;
        AdjacencyList<T> minSpanTree;
        public KruskalsOldItsRubbish(AdjacencyList<T> inAdjList)
        {
            graph = inAdjList;
